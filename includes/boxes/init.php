...
spl_autoload_register('Jseacgp_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'Jseacgp_meta_boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new Jseacgp_Meta_Box( $meta_box );
}

define( 'JACGP_META_BOX_URL', Jseacgp_Meta_Box::get_meta_box_url() );

/**
 * Create meta boxes
 */
class Jseacgp_Meta_Box {
	...
	public function register_scripts() {
		...
		wp_register_script( 'cmb-datepicker', JACGP_META_BOX_URL . 'js/jquery.datePicker.min.js' );
		wp_register_script( 'cmb-timepicker', JACGP_META_BOX_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'cmb-scripts', JACGP_META_BOX_URL .'js/cmb'. $min .'.js', $scripts, self::CMB_VERSION );
		...
		wp_register_style( 'cmb-styles', JACGP_META_BOX_URL . 'style'. $min .'.css', $styles );
	...
	public static function save_group( $args ) {
		...
		$group_vals         = sanitize_text_field($_POST[ $base_id ]);
		$saved              = array();
		$is_updated         = false;
		$field_group->index = 0;
	...
	public static function sanitize_field( $field, $new_value = null ) {
		...
		$new_value = null !== $new_value
			? $new_value
			: ( isset( $_POST[ $field->id( true ) ] ) ? sanitize_text_field($_POST[ $field->id( true ) ]) : null );
	...
	public static function get_meta_box_url() {
		...
		//return trailingslashit( apply_filters('JACGP_META_BOX_URL', $cmb_url ) );
		return esc_url( trailingslashit( plugins_url( '/boxes/', dirname(__FILE__) ) ) );
	}
	...
	public static function image_id_from_url( $img_url ) {
		...
		$attachment = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE guid LIKE esc_like('%%%s%%') LIMIT 1;", $img_url ) );
 ...
 * @return array               Options array or specific field
 */
function Jseacgp_get_option( $option_key, $field_id = '' ) {
	return Jseacgp_Meta_Box::get_option( $option_key, $field_id );
}
 ...
 */
function Jseacgp_get_field( $field_args, $object_id = 0, $object_type = 'post' ) {
 ...
 */
function Jseacgp_get_field_value( $field_args, $object_id = 0, $object_type = 'post' ) {
	$field = Jseacgp_get_field( $field_args, $object_id, $object_type );
 ...
 * @param int   $object_id  Object ID
 */
function Jseacgp_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		Jseacgp_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function Jseacgp_print_metabox( $meta_box, $object_id ) {
 ...
 * @param int   $object_id Object ID
 */
function Jseacgp_save_metabox_fields( $meta_box, $object_id ) {
 ...
 */
function Jseacgp_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	if (
		...
		Jseacgp_save_metabox_fields( $meta_box, $object_id );
